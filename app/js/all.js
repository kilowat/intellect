...
$label.html(labelVal);});$input.on('focus',function(){$input.addClass('has-focus');}).on('blur',function(){$input.removeClass('has-focus');});});})(jQuery,window,document);$('.action-menu').hover(function(){var self=$(this);var t;self.find('.action-menu-block').fadeIn(10);self.addClass('selected');self.mouseleave(function(){t=setTimeout(function(){self.find('.action-menu-block').fadeOut('fast',function(){self.find('.action-menu-block').clearQueue();self.removeClass('selected');});},10)});self.find('.action-menu-block').hover(function(){clearTimeout(t);});});});